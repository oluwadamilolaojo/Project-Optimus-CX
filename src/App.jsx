...
import {
  LayoutDashboard, Users, TrendingDown, DollarSign, Search,
  SlidersHorizontal, X, ChevronDown, Info, Building2, RotateCcw,
} from "lucide-react";
...
/* ============================== DATA ============================== */
const RAW = [
  ["GiftHealth (all lines)","", "Large",551,572,508,11,37,0,21,0.04,"Under","Over","Within","Optimized","","",19.34,"Weighted blend of GiftHealth lines",527,11,37,0,0.70,true,false],
  ...
  ["9am Health","","Large",32,27,25,0,2,0,-5,-0.16,"—","Over","Under","Optimized","","",34.18,"US account — $34.18 billed / $20 cost",25,0,2,0,0.70,false,true],
  ...
  ["Earnest RCM","Kestrel","Small",4,4,3,0,1,0,0,0.0,"—","Over","Within","Optimized","Right-sized in June: billable cut 11 to 4, rate raised to ~$14. TL 1:3 is a small-account floor (one billable TL over three agents), not over-staffing. Shrinkage 0%. Optimized post-resize.","Lydia",15,"Rate card",3,0,1,0,0.80,false,false],
...
function recompute(acct, A) {
  ...
  if (acct.gh) {
    ...
    return { rev, cost, margin: (rev - cost) / rev, rateEff: rev / (bill * H), costHr: cost / (act * H), bill, act };
  }
  if (acct.nineam) {
    const rev = 34.18 * acct.bill * H, cost = A.us * acct.act * H;
    return { rev, cost, margin: (rev - cost) / rev, rateEff: 34.18, costHr: A.us, bill: acct.bill, act: acct.act };
  }
  const rev = acct.rate * acct.bill * H;
  const cost = (acct.mAg * A.agent + acct.mQ * A.qaosme + acct.mT * A.tl + acct.mP * A.pc) / FX;
  return { rev, cost, margin: (rev - cost) / rev, rateEff: acct.rate, costHr: cost / (acct.act * H), bill: acct.bill, act: acct.act };
}
...
const DEC_STYLE = {
  Optimized: [T.green, T.greenBg], "To Discuss": [T.amber, T.amberBg], "Special Circumstances": [T.navy, "#E7ECF5"],
};

/* ============================== ATOMS ============================== */
const Pill = ({ v }) => {
  const [fg, bg] = V_STYLE[v] || V_STYLE["—"];
  return <span className="inline-flex items-center rounded-full px-2 py-0.5" style={{ background: bg, color: fg, fontSize: 11, fontWeight: 600, letterSpacing: 0.2 }}>{v}</span>;
};
const Badge = ({ d }) => {
  const [fg, bg] = DEC_STYLE[d] || [T.slate, T.slateBg];
  const short = d === "Special Circumstances" ? "Special" : d;
  return <span className="inline-flex items-center rounded-md px-2 py-1" style={{ background: bg, color: fg, fontSize: 11, fontWeight: 600 }}>{short}</span>;
};
const SizeTag = ({ s }) => <span className="inline-flex items-center rounded px-1.5 py-0.5" style={{ background: "#EEF2F8", color: T.navy, fontSize: 10.5, fontWeight: 600 }}>{s}</span>;

function CommentCell({ value, onChange, source, w = 250 }) {
  return (
    <div style={{ width: w }} onClick={(e) => e.stopPropagation()}>
      {source && source !== "Note" && (
        <div style={{ fontSize: 9.5, color: T.faint, fontWeight: 600, marginBottom: 3, textAlign: "left" }}>from {source}</div>
      )}
      {source === "Note" && (
        <div style={{ fontSize: 9.5, color: T.faint, fontWeight: 600, marginBottom: 3, textAlign: "left" }}>structural note</div>
      )}
      <textarea
        value={value}
        ...
        rows={2}
        style={{
          width: "100%", minHeight: 40, resize: "vertical", border: `1px solid ${T.line}`,
          borderRadius: 8, padding: "6px 8px", fontFamily: FONT, fontSize: 11.5, color: T.ink,
          lineHeight: 1.45, outline: "none", background: "#fff", textAlign: "left", display: "block",
        }}
...
export default function App() {
  ...
  const [decF, setDecF] = useState(new Set());
  ...
  useEffect(() => {
    try { localStorage.setItem(LS_KEY, JSON.stringify(comments)); } catch {}
  }, [comments]);
  const setComment = (id, text) => setComments((p) => ({ ...p, [id]: text }));

  const money = (n) => (cur === "USD" ? usd(n) : ngn(n * A.fx));
  const computed = useMemo(() => ACCOUNTS.map((a) => ({ ...a, m: recompute(a, A) })), [A]);
  const filtered = useMemo(() => computed.filter((a) => {
    ...
    if (decF.size && !decF.has(a.decision)) return false;
    return true;
  }), [computed, search, sizeF, decF]);

  const portfolio = useMemo(() => {
    const rev = computed.reduce((s, a) => s + a.m.rev, 0);
    const cost = computed.reduce((s, a) => s + a.m.cost, 0);
    const byDec = {};
    computed.forEach((a) => (byDec[a.decision] = (byDec[a.decision] || 0) + 1));
    const bad = computed.filter((a) => marginVerdict(a.m.margin, a.bench) === "Bad").length;
    return { rev, cost, margin: (rev - cost) / rev, byDec, bad, n: computed.length };
  }, [computed]);

  const toggle = (set, fn, v) => { const n = new Set(set); n.has(v) ? n.delete(v) : n.add(v); fn(n); };
  const NAV = [["overview", "Overview", LayoutDashboard], ["loading", "Loading Ratios", Users], ["shrinkage", "Shrinkage", TrendingDown], ["margin", "Margin", DollarSign]];
      ...
      <div className="flex" style={{ minHeight: "100vh" }}>
        ...
        <aside className="flex flex-col justify-between" style={{ width: 232, background: T.navy, color: "#fff", position: "sticky", top: 0, height: "100vh" }}>
          <div>
            <div className="px-5 pt-6 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.10)" }}>
              <div className="flex items-center gap-2">
                <div className="flex items-center justify-center rounded-lg" style={{ width: 30, height: 30, background: T.blue }}><Building2 size={17} color="#fff" /></div>
                <div style={{ fontWeight: 700, fontSize: 16, letterSpacing: -0.2 }}>Hugo CX</div>
              </div>
              <div style={{ fontSize: 11, color: "#9DB0CE", marginTop: 8, lineHeight: 1.4 }}>Project Phoenix · Loading, Shrinkage & Margin command center</div>
            </div>
            <nav className="px-3 pt-4">
              {NAV.map(([k, label, Icon]) => (
                <button key={k} onClick={() => setView(k)} className="flex items-center gap-3 w-full rounded-lg px-3 py-2.5 mb-1"
          ...
          <div className="px-3 pb-4">
            <button onClick={() => setDrawer(true)} className="flex items-center gap-3 w-full rounded-lg px-3 py-2.5 mb-2" style={{ background: "rgba(255,255,255,0.06)", color: "#DCE5F2", fontSize: 13, fontWeight: 600 }}>
        ...
        <main className="flex-1" style={{ minWidth: 0 }}>
          <div className="flex items-center gap-3 px-6 py-3 flex-wrap" style={{ position: "sticky", top: 0, zIndex: 20, background: "rgba(246,248,252,0.92)", backdropFilter: "blur(8px)", borderBottom: `1px solid ${T.line}` }}>
            <div className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: "#fff", border: `1px solid ${T.line}`, flex: "1 1 240px", maxWidth: 340 }}>
            ...
            <FilterGroup label="Status" opts={["Optimized", "To Discuss", "Special Circumstances"]} set={decF} onToggle={(v) => toggle(decF, setDecF, v)} short />
            <div className="flex items-center rounded-lg overflow-hidden" style={{ border: `1px solid ${T.line}`, background: "#fff" }}>
            ...
            {(sizeF.size || decF.size || search) > 0 && (
              <button onClick={() => { setSizeF(new Set()); setDecF(new Set()); setSearch(""); }} className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg" style={{ color: T.sub, fontSize: 12, fontWeight: 600 }}>
          ...
          <div className="px-6 py-6">
            ...
            {view === "loading" && <Loading rows={filtered} lens={lens} setLens={setLens} openRow={openRow} setOpenRow={setOpenRow} comments={comments} setComment={setComment} />}
...
/* ============================== TOP-BAR FILTER ============================== */
function FilterGroup({ label, opts, set, onToggle, short }) {
  return (
    <div className="flex items-center gap-1.5">
      <span style={{ fontSize: 11, color: T.faint, fontWeight: 600 }}>{label}</span>
      {opts.map((o) => {
        const on = set.has(o);
        const lab = short && o === "Special Circumstances" ? "Special" : short && o === "To Discuss" ? "Discuss" : o;
        return (
          <button key={o} onClick={() => onToggle(o)} className="rounded-full px-2.5 py-1" style={{ fontSize: 11.5, fontWeight: 600, border: `1px solid ${on ? T.navy : T.line}`, background: on ? T.navy : "#fff", color: on ? "#fff" : T.sub }}>{lab}</button>
        );
      })}
    </div>
  );
...
function Overview({ portfolio, money, computed, onSelect }) {
  const Card = ({ children, style }) => <div className="rounded-xl p-5" style={{ background: "#fff", border: `1px solid ${T.line}`, ...style }}>{children}</div>;
  const flagged = computed.filter((a) => marginVerdict(a.m.margin, a.bench) === "Bad");
  return (
    <div>
      ...
      <div className="grid gap-4 mt-5" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))" }}>
        ...
        <Kpi label="Accounts below benchmark" value={portfolio.bad} accent={T.red} sub="margin verdict · Bad" />
      </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))" }}>
        {SCOPES.map((s) => (
          <Card key={s.name}>
            ...
            <div className="flex gap-4 mt-3" style={{ fontSize: 12, color: T.sub }}>
      ...
      <h2 style={sec}>Needs attention</h2>
      <Card style={{ padding: 0 }}>
        {flagged.length === 0 ? (
          <div className="p-5" style={{ color: T.sub, fontSize: 13 }}>Every account is at or above its margin benchmark. Adjust the assumptions to stress-test.</div>
        ) : flagged.map((a) => (
          <button key={a.id} onClick={() => onSelect(a)} className="hx-row flex items-center justify-between w-full px-5 py-3" style={{ borderTop: `1px solid ${T.line}`, textAlign: "left" }}>
            <div className="flex items-center gap-3"><SizeTag s={a.size} /><span style={{ fontWeight: 600, fontSize: 13.5 }}>{a.name}</span><Badge d={a.decision} /></div>
            <div className="flex items-center gap-4"><span style={{ fontSize: 12, color: T.sub }}>margin {pct(a.m.margin)} · target {pct(a.bench)}</span><Pill v="Bad" /><Info size={15} color={T.faint} /></div>
          </button>
        ))}
      </Card>
    </div>
...
const Kpi = ({ label, value, accent, sub }) => (
  <div className="rounded-xl p-5" style={{ background: "#fff", border: `1px solid ${T.line}`, borderTop: `3px solid ${accent}` }}>
...
/* ============================== LOADING ============================== */
function Loading({ rows, lens, setLens, openRow, setOpenRow, comments, setComment }) {
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: -0.4 }}>Loading ratios</h1>
          <p style={{ color: T.sub, fontSize: 13.5, marginTop: 4 }}>QA and TL load per account against the {lens === "segment" ? "segment-specific Hugo ideal" : "blanket industry band"}. The notch marks the ideal; the comment box on the right is editable.</p>
        </div>
        <div className="flex items-center rounded-lg overflow-hidden" style={{ border: `1px solid ${T.line}`, background: "#fff" }}>
      ...
      <div className="rounded-xl mt-5 overflow-x-auto" style={{ background: "#fff", border: `1px solid ${T.line}` }}>
        <table className="w-full" style={{ borderCollapse: "collapse", minWidth: 1080 }}>
          <thead><tr>{["Account", "Size", "Bill", "Act", "QA load", "TL load", "Shrink", "Status", "Comment / Rationale"].map((h, i) => (<th key={i} style={{ ...th, textAlign: i === 0 || i === 8 ? "left" : "center", position: "sticky", top: 0 }}>{h}</th>))}</tr></thead>
          <tbody>
            {rows.map((a) => {
                ...
                <React.Fragment key={a.id}>
                  ...
                  {open && (
                    <tr style={{ background: "#FAFCFF" }}>
                      <td colSpan={9} style={{ padding: "16px 20px", borderTop: `1px solid ${T.line}` }}>
                        <div className="grid gap-5" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))" }}>
                          ...
                          <Detail k="Shrinkage" v={pct(a.shrinkPct)} sub={`target ${pct(SH_TARGET[a.size])}`} />
                        </div>
                      </td>
                    </tr>
                  )}
...
function Shrinkage({ rows, onSelect, comments, setComment }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: T.sub, fontSize: 13.5, marginTop: 4 }}>Actual vs billable, sorted by distance from the segment target (Large 15% · Medium 20% · Small 25%). The notch marks the target. Edit the note on the right; click a name for the full read.</p>
      <div className="rounded-xl mt-5 p-4 overflow-x-auto" style={{ background: "#fff", border: `1px solid ${T.line}` }}>
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 py-2" style={{ borderTop: `1px solid ${T.line}` }}>
                <button onClick={() => onSelect(a)} className="flex items-center gap-2" style={{ width: 200, flexShrink: 0, textAlign: "left" }}>
                  <SizeTag s={a.size} />
                  <span style={{ fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{a.name}</span>
                </button>
                <div style={{ flex: 1, minWidth: 120, height: 18, background: "#F1F5FA", borderRadius: 5, position: "relative", overflow: "hidden" }}>
                  {a.shrinkPct >= 0
    ...
    <div>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p style={{ color: T.sub, fontSize: 13.5, marginTop: 4 }}>Revenue, cost and margin per account against its benchmark. Verdict band is ±3%: above is Great, inside is Good, below is Bad. The comment box sits beside the verdict.</p>
        </div>
        <button onClick={onOpenDrawer} className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: T.navy, color: "#fff", fontSize: 12.5, fontWeight: 600 }}><SlidersHorizontal size={15} /> Assumptions</button>
      </div>
              ...
              return (
                <React.Fragment key={a.id}>
                  <tr className="hx-row" style={{ borderTop: `1px solid ${T.line}`, cursor: a.gh ? "pointer" : "default" }} onClick={() => a.gh && setOpenRow(open ? null : "m" + a.id)}>
                    ...
                    <td style={tdC}><span style={{ fontWeight: 700 }}>{pct(a.m.margin)}</span></td>
  ...
  return (
    <tr style={{ background: "#FAFCFF" }}>
      <td colSpan={10} style={{ padding: "12px 20px", borderTop: `1px solid ${T.line}` }}>
        ...
        <table className="w-full" style={{ borderCollapse: "collapse" }}>
          <thead><tr>{["Line", "Geography", "Rate", "Bill", "Act", "Basis", "Revenue", "Cost"].map((h, i) => (<th key={i} style={{ ...thSm, textAlign: i === 0 || i === 1 ? "left" : "center" }}>{h}</th>))}</tr></thead>
          <tbody>
            {lines.map((l) => (
    ...
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, height: "100vh", width: 380, maxWidth: "92vw", background: "#fff", zIndex: 50, boxShadow: "-12px 0 32px rgba(15,30,56,0.16)", overflowY: "auto" }}>
        <div className="px-5 py-4 flex items-start justify-between" style={{ borderBottom: `1px solid ${T.line}`, position: "sticky", top: 0, background: "#fff" }}>
          <div>
            <div className="flex items-center gap-2"><SizeTag s={a.size} /><Badge d={a.decision} /></div>
        ...
        <div className="px-5 py-4">
          ...
          <div className="grid gap-3 mt-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
            ...
            <Mini k="Rate $/hr" v={"$" + a.m.rateEff.toFixed(a.gh || a.nineam ? 2 : 0)} /><Mini k="Benchmark" v={pct(a.bench)} />
          </div>
          <div className="mt-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              {a.source === "Note" && <span style={{ fontSize: 10.5, color: T.faint }}>structural note</span>}
            </div>
            <textarea value={comments[a.id] || ""} onChange={(e) => setComment(a.id, e.target.value)} placeholder="Add a note…" rows={6}
              style={{ width: "100%", resize: "vertical", border: `1px solid ${T.line}`, borderRadius: 10, padding: "10px 12px", fontFamily: FONT, fontSize: 13, lineHeight: 1.55, color: T.ink, outline: "none", background: T.paper }}
...
const Mini = ({ k, v }) => <div className="rounded-lg p-3" style={{ background: T.paper }}><div style={lblS}>{k}</div><div style={{ fontWeight: 700, fontSize: 14, marginTop: 2 }}>{v}</div></div>;
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, height: "100vh", width: 360, maxWidth: "92vw", background: "#fff", zIndex: 50, boxShadow: "-12px 0 32px rgba(15,30,56,0.16)", overflowY: "auto" }}>
        ...
        <div className="px-5 py-4">
          <p style={{ fontSize: 12.5, color: T.sub, lineHeight: 1.5, marginBottom: 16 }}>Every figure recalculates across all accounts the moment you change a value. Revenue = rate × billable × hours. Cost = actual-basis salaries ÷ FX.</p>
          {fields.map(([k, label, step]) => (
            <div key={k} className="mb-4">
              <label style={{ fontSize: 11.5, color: T.sub, fontWeight: 600 }}>{label}</label>
              <input type="number" step={step} value={A[k]} onChange={(e) => setA({ ...A, [k]: Number(e.target.value) || 0 })} className="w-full rounded-lg px-3 py-2 mt-1.5"
                style={{ border: `1px solid ${T.line}`, fontFamily: FONT, fontSize: 14, fontWeight: 600, color: T.blue, outline: "none" }} />
            </div>
          ))}
          <button onClick={() => setA({ ...DEFAULTS })} className="flex items-center gap-2 rounded-lg px-3 py-2 mt-2" style={{ border: `1px solid ${T.line}`, color: T.sub, fontSize: 12.5, fontWeight: 600, width: "100%", justifyContent: "center" }}>
